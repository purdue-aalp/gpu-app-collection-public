#!/bin/bash
# Whisper Large V3 inference wrapper for Accel-Sim run_hw_trace.py
# Arguments: $1 = audio_len (seconds)

set -e

AUDIO_LEN="${1:-30}"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TRACER_DIR="${SCRIPT_DIR}/../tracer_nvbit"

source "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/common_env.sh"


export PYTHONPATH="${TRACER_DIR}/others:${TRACER_DIR}/tracer_tool:${PYTHONPATH}"
export NVBIT_INSTRUMENTATION_ENABLED=0
export ALLOW_REG_VAL_TRACING=1

echo "=== Whisper Large V3 Inference ==="
echo "  audio_len: ${AUDIO_LEN}s"
echo "  TRACES_FOLDER: ${TRACES_FOLDER}"

python3 "${SCRIPT_DIR}/whisper_inference_trace.py" \
    --audio_len "${AUDIO_LEN}" \
    --num_batches 1
