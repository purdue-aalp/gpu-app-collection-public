...
//
// Smem layout: canonical CuTe GMMA Major-K tiling. In uint128_t units:
...
// (SBO=128*Wsw, LBO=16).
//
// On top of that offset, real Hopper hardware additionally applies an XOR
// swizzle -- CuTe's Swizzle<B,4,3>: XOR bits [7,7+B) of the byte offset into
// bits [4,4+B), with B=3/2/1 for 128B/64B/32B. This was previously believed
// to be a bank-conflict-only optimization safe to omit on both producer and
// consumer (matching GPGPU-Sim's own simplified wgmma_apply_swizzle(), which
// uses a coarser block-parity XOR that is itself NOT hardware-accurate); on
// real silicon the XOR is load-bearing for correctness, not just performance
// -- verified against the vendored CUTLASS/CuTe GMMA descriptor atoms
// (cutlass-bench) and against real H100/H200 hardware (see apply_sw below).
//
...
// Canonical CuTe GMMA Major-K smem byte offset (pre-swizzle). In units of
// uint128_t:
//   u128 = (leading/T)*(LBO/16) + (stride%8)*Wsw + (stride/8)*(SBO/16)
...
// K-major operand tile whose MN extent is MN.
__host__ __device__ __forceinline__ constexpr
...
// byte offset into bits [4,4+B). B = 3/2/1 for 128B/64B/32B swizzle. This is
// load-bearing for correctness on real hardware, not just a bank-conflict
// optimization -- verified against the vendored CUTLASS/CuTe GMMA descriptor
// atoms (cutlass-bench) and against real H100/H200 hardware.
...
__global__ void kernel_f16(const half* A_g, const half* B_g,
  ...
  for (int i = tid; i < M*K; i += WGSIZE) {
    int m = i/K, k = i%K;
    *(half*)(smA + smem_off(k, m, E, LBO_A, SW)) = A_g[i];
  }
  for (int i = tid; i < K*N; i += WGSIZE) {
    int k = i/N, n = i%N;
    *(half*)(smB + smem_off(k, n, E, LBO_B, SW)) = B_g[i];
  }
  __syncthreads();
